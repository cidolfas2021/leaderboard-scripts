...
namespace GameZero
{
    public partial class LeaderboardEntry
    {
        ...
        public int Level => entry.m_nLevel;
        public int Gold => entry.m_nGold;
        public int lifetimePlayerKills => entry.m_nlifetimePlayerKills;
        public int thismatchPlayerKills => entry.m_nthismatchPlayerKills;
        public int GoonKills => entry.m_nGoonKills;
        public int BossKills => entry.m_nBossKills;
        public int lifetimedeaths => entry.m_nlifetimedeaths;
        public int thismatchdeaths => entry.m_nthismatchdeaths;
        ...
        public void GetAttachedUgc<T>(Action<T, bool> callback = null)
        {
            ...
            {
                HeathenEngineering.SteamworksIntegration.API.RemoteStorage.Client.UGCDownload(UgcHandle, 0, (dr, de) =>
                {
                    if (!de && dr.m_eResult == EResult.k_EResultOK)
                    {
                        ...
                        if (callback != null)
                        {
                            var buffer = HeathenEngineering.SteamworksIntegration.API.RemoteStorage.Client.UGCRead(dr.m_hFile);
        ...
        public bool StartUgcDownload(uint priority = 0)
        {
            if (UgcHandle != UGCHandle_t.Invalid)
            {
                HeathenEngineering.SteamworksIntegration.API.RemoteStorage.Client.UGCDownload(UgcHandle, priority, HandleUgcDownloadResult);
                return true;
            }
            else
                return false;
        ...
        public bool StartUgcDownload(uint priority, Action<RemoteStorageDownloadUGCResult_t, bool> callback)
        {
            if (UgcHandle != UGCHandle_t.Invalid)
            {
                HeathenEngineering.SteamworksIntegration.API.RemoteStorage.Client.UGCDownload(UgcHandle, priority, (p, e) =>
                {
